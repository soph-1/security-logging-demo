...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['register'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($register_sql->execute() === TRUE) {
            ...
            logMessage("INFO", "Registrazione avvenuta con successo", sanitizeForLog($nome_utente));
            header("Location: forum.php");
            exit;
        } else {
            logMessage("ERROR", "Errore durante la registrazione", sanitizeForLog($nome_utente));
    ...
    if ($login_result->num_rows > 0) {
        ...
        if (password_verify($password, $row['password'])) {
            $_SESSION['logged_in'] = true;
            $_SESSION["nome_utente"] = $nome_utente;
            logMessage("INFO", "Accesso riuscito", sanitizeForLog($nome_utente));
            header("Location: forum.php");
            exit;
        } else {
            logMessage("ERROR", "Accesso negato", sanitizeForLog($nome_utente));
